...
{
	add_action( 'admin_enqueue_scripts', array('GRAV_CACHE_PLUGIN_SETTINGS', 'add_sortable') );
}

class GRAV_CACHE_PLUGIN_SETTINGS
{
					...
					if($fields[$key]['type'] == 'repeater' && is_array($value))
					{
						...
						foreach ($value as $rep_i => $rep_values)
						{
							...
							foreach ($rep_original_fields as $rep_key => $rep_value)
							{
								...
								{
									$fields[$key]['fields'][$rep_i][$rep_key]['value'] = stripslashes($rep_values[$rep_key]);
								}
							}
						}
					}
	...
	public static function get_form($fields)
	{
		...
		?>
			...
			<style>
				...
				.grav-plugin-settings-form .description {display: block; }
